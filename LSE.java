...
public class LSE {
	class No {
		...
		public void createUI () {
			this.noLista = new NoLista("" + conteudo);
		}
	...
	public void createUI(Group pane, int pos) {
		...
		for ( int i = 1; i < pos + 1; i++) {
			if (i == pos) {
				aux.noLista.setLayoutX((70 + 70 + 35) * pos);
				pane.getChildren().addAll(aux.noLista);
				No aux2 = aux;
				for (int j = pos; j < nElementos + 1; j++) {
					aux2.noLista.setLayoutX(aux2.noLista.getLayoutX() + (70 + 70 + 35));
					aux2 = aux2.getProx();
				}
			}
			aux = aux.getProx();
	...
	public void removeUI(Group pane, int pos) {
		...
		for ( int i = 1; i < pos + 1; i++) {
			if (i == pos) {
				aux2 = aux;
				pane.getChildren().removeAll(aux.noLista);
				aux2 = aux2.getProx();
